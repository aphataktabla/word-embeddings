    ...
    for bf in list_bucket_files(cooc_dir):
        with bf.open("rb") as f:
            while True:
                ...
                vals.append(arr["c"][mask].astype(np.float32, copy=False))
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Run sparse PCA on a restricted submatrix of the raw co-occurrence matrix. "
            "This is intended for a vocab subset, not the full 300k x 300k matrix."
        )
    )
    ap.add_argument(
        "--out_dir",
    ...
    stem = f"cooc_sparse_pca_v{cutoff}_k{args.n_components}"
